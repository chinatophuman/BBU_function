...
class PCIE_test:
    def __init__(self,logname,hostname,port,username,password):
        ...
        if ('I210' not in pcie_Ethernet_info):
        ...
        elif ('AST1150' not in pcie_chip_info):
        ...
        elif ('AMD' not in pcie_external_graphic_card):
            print('Pcie Test failed, detect external graphic card failed, error coed is 13003')
            with open(logname, 'a+') as f:
                f.write("Pcie Test failed, detect external graphic card failed, error coed is 13003\r")
        else:
            PCIE_result='PASS'
